...
import { ForumPostSchemaType, forumPostSchema } from "@/app/api/forum/schema";
...
const CreatePostModal = ({ clickModal }: CreatePostParams) => {
  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<ForumPostSchemaType>({ resolver: zodResolver(forumPostSchema) });
  const mutation = useMutation({
    mutationFn: (postData: ForumPostSchemaType) => {
      return createPost(postData);
    },
  });

  const queryClient = useQueryClient();
  const onSubmit: SubmitHandler<ForumPostSchemaType> = async (data) => {
